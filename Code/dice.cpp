		...
		case Event::MouseButtonPressed:
		{
			...
			if (toakText.getGlobalBounds().contains(mousePosF) && (board1[6] == -1) && rolled == true && turn == 1)
			{
				...
				for (int i = 0; i < 4; i++)
				{
					...
					if (tally >= 2) {
						if (diceResult[i] * tally > totals) {
							totals = diceResult[i] * tally;
						}
					}
					tally = 0;
				}
			...
			if (houseText.getGlobalBounds().contains(mousePosF) && (board1[8] == -1) && rolled == true && turn == 1)
			{
				...
				turn = 2;
				totals = 0;
				int tally = 0, check1 = 0, check2 = 0, hold1 = 99, hold2 = 99;
				std::sort(diceResult, diceResult + 5);
				for (int i = 0; i < 4; i++)
				{
					for (int j = i + 1; j < 5; j++)
					{
					...
					if (tally == 1 && diceResult[i] != hold1 && diceResult[i] != hold2) {
						check1 = 1;
						hold1 = diceResult[i];
					}
					if (tally == 2 && diceResult[i] != hold2) {
						check2 = 1;
						if (diceResult[i] == hold1) {
							hold1 = 99;
							check1 = 0;
						}
						hold2 = diceResult[i];
