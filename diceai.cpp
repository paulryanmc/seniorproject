...
void diceai() {
	...
	int diceResult[5] = { 0 };
	int diceResultProxy[5] = { 0 };
	int diceFreeze[5] = { 0 };
	srand(time(0));
	int rollCount = 0, totals = 0, score = 0, check = 0, once = 0, aiturn = 0;
	bool gamecomplete = false, rolled = false, reroll = true;
	...
	while (diceWindow.isOpen())
	{
		...
		if (turn == 2 && gamecomplete == false) {
			tally = 0, tally2 = 0, tally3 = 0;
			aiturn = 90;
			do {
				totals = 0;
				for (int i = 0; i < 5; ++i) {
				...
				if (tally2 >= 5 && board2[11] == -1) {
					aiturn = 11;
				}
				else if (tally2 >= 3 && board2[7] == -1) {
					aiturn = 7;
				}
				else if (tally2 >= 2 && board2[6] == -1) {
					aiturn = 6;
				}
				else {
					for (int i = 0; i < 4; i++)
					...
					if (tally3 >= 4 && board2[10] == -1) {
						aiturn = 10;
					}
					else if (tally3 >= 3 && board2[9] == -1) {
						aiturn = 9;
					}
				}
				...
				if (aiturn == 90) {
					...
					for (int i = 0; i < 4; i++)
					{
						for (int j = i + 1; j < 5; j++)
						{
							if (diceResult[i] == diceResult[j] && diceResultProxy[j] != 17) {
								tally++;
								diceResultProxy[j] = 17;
							}
						}
						if (tally > tally2) {
							if (tally3 == 0) {
					...
					if (tally3 + tally2 == 5 && board2[8] == -1) {
						aiturn = 8;
					}
					else if (totals > 17 && board2[12] == -1) {
						aiturn = 12;
					}
					else {
						for (int i = 0; i < 12; i++) {
